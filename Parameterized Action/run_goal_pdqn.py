...
@click.option('--load-model', default=0, help="Do you want to load the model", type=int)
@click.option('--load-dir', default="../new_result/goal/PDQN0/", help="directory to load the model", type=str)
@click.option('--index-model', default=1, help="directory to load the model", type=int)
@click.option('--save-video', default="/video/test", help="Give Location to Save the Frames", type=str)
def run(seed, episodes, evaluation_episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold,
        ...
        action_input_layer, initialise_params, save_freq, save_dir, save_frames, visualise, load_model, load_dir, index_model, save_video):
    ...
    if save_frames:
        assert render_freq > 0
        vidir = os.path.join(save_video, "frames")
    ...
    for i in range(episodes):
        ...
        # if load_model > 0 and index_model:
        #     agent.load_models(os.path.join(load_dir, str(index_model)))
